...
public class VKPoster extends TelegramLongPollingBot
{
  ...
  private static final String VK_CODE = "dce735bf9cdf76509f";
  ...
  public void onUpdateReceived( Update update )
  {
    if ( update.hasMessage() && update.getMessage().hasText() ) {
      ...
      if ( MASTER_CHAT_ID.equals( chatId ) ) {
        ...
        String stringUrl = update.getMessage().getText();
        ...
        try {
          ...
          sendTextMessageToUser( String.format( MALFORMED_URL_ERROR_TEXT, stringUrl, e.getMessage() ), chatId );
          e.printStackTrace();
          return;
        }
        ...
        if ( ImageTypeEnum.IMAGE_TYPE_UNKNOWN.equals( imageType ) ) {
          sendTextMessageToUser( String.format( UNSUPPORTED_IMAGE_FORMAT_ERROR_TEXT, stringUrl ), MASTER_CHAT_ID );
          return;
        }
        ...
        try {
          ...
          if(postId != null){
            sendTextMessageToUser(String.format( UPLOAD_SUCCESSFUL_MESSAGE_TEXT, postId ), chatId);
          }
        } catch (ClientException e) {
          sendTextMessageToUser( String.format( NOT_THE_MASTER_INFO_TEXT, e.getMessage() ), MASTER_CHAT_ID );
          e.printStackTrace();
        ...
        sendTextMessageToUser( String.format( NOT_THE_MASTER_INFO_TEXT, userName ), MASTER_CHAT_ID );
      }
    }
  }
